...
class Router {
    ...
    async handleNavigation() {
        ...
        if (!authManager && page !== 'home') {
            this.navigate('home');
            return;
        }

        if (authManager && ['dashboard', 'exercise', 'profile'].includes(page) && !authManager.isAuthenticated()) {
            this.navigate('login');
            return;
        }

        if (authManager && ['login', 'signup'].includes(page) && authManager.isAuthenticated()) {
    ...
    setupLoginPage() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // Guard de chargement: évite les clics avant que Supabase soit prêt
            ...
            if (!emailEl || !passEl) {
                errorDiv.textContent = 'Champs de connexion manquants.';
                errorDiv.classList.remove('hidden');
                return;
            }
            ...
            if (result.success) {
                ui.showNotification('Connexion reussie!', 'success');
                this.navigate('dashboard');
            } else {
    ...
    setupSignupPage() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (result.success) {
                ui.showNotification('Inscription reussie!', 'success');
                this.navigate('dashboard');
            } else {
                errorDiv.textContent = result.error;
                errorDiv.classList.remove('hidden');
